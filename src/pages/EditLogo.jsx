...
export default function EditLogo() {
  ...
  const fetchLogo = async () => {
    try {
      ...
      setValue("title", logo.title || "");
      setPreview(
        logo.image
          ? `${import.meta.env.VITE_API_BASE_URL}${logo.image}`
          : null
      );
    } catch (error) {
      console.error(error);
  ...
  const onSubmit = async (data) => {
    try {
      ...
      await api.put("/admin/logo/updateLogo", formData, {
      ...
      <div className="flex-1 flex flex-col">
        ...
        <main className="flex-1 overflow-y-auto p-6">
          <div className="bg-white rounded-xl shadow-sm border p-6 w-full">
            ...
            <h2 className="text-2xl font-semibold mb-6">Edit Logo</h2>
  ...
  name="title"
  control={control}
  rules={{ required: "Logo title is required" }}
  render={({ field }) => (
    <input
      {...field}
